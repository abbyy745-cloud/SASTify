...
class ResultsPanel {
    static show(extensionUri, scanResults, provider) {
    ...
    static createOrShow(extensionUri, scanResults, provider) {
        ...
        const panel = vscode.window.createWebviewPanel('sastifyResults', 'SASTify Security Results', column, {
            enableScripts: true,
    ...
    constructor(panel, extensionUri, scanResults, provider) {
        this._disposables = [];
        this._scanHistory = [];
        this._panel = panel;
    ...
    async handleAIAnalysis(issueIndex, codeSnippet) {
        try {
    ...
    async handleFalsePositiveReport(issueIndex, comment) {
    ...
    generatePremiumHtmlReport(issues) {
        const sc = { Critical: 0, High: 0, Medium: 0, Low: 0 };
        for (const i of issues) {
            if (sc[i.severity] !== undefined) {
                sc[i.severity]++;
            }
        }
...
<body>
    ...
    async applyFixToEditor(fix, explanation, issueIndex) {
        ...
        try {
            ...
            const range = new vscode.Range(document.positionAt(startOffset + relativeIndex), document.positionAt(startOffset + relativeIndex + issue.snippet.length));
    ...
    _update(scanResults, filter) {
        const MAX = 100;
        let displayResults = Object.assign({}, scanResults);
        if (scanResults.issues && scanResults.issues.length > MAX) {
            const order = { 'Critical': 0, 'High': 1, 'Medium': 2, 'Low': 3 };
            const sorted = scanResults.issues.slice().sort((a, b) => (order[a.severity] !== undefined ? order[a.severity] : 4) -
